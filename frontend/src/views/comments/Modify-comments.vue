<template>
    <div class="card-groupomania">
        <h1><b-icon-pencil-square></b-icon-pencil-square> Modifier le commentaire</h1>

        <b-form @submit.prevent="submitForm" class="form">
            <b-form-group>
                <b-form-textarea placeholder="Commentaire" rows="4" max-rows="6" autofocus v-model="$v.content.$model" :class="{ 'is-invalid' : $v.content.$error, 'is-valid' : !$v.content.$invalid }" ></b-form-textarea>
            ...
            <b-button type="submit"  @click="redirectionComment()" style="border-radius:20px" variant="success" :class="{ 'disabled' : invalidateFields }"> Modifier</b-button>
        </b-form>
    </div>
</template>
